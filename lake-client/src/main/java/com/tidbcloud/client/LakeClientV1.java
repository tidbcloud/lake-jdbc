...
@ThreadSafe
public class LakeClientV1
        implements LakeClient {
    private final AtomicReference<Boolean> finished = new AtomicReference<>(false);
    public static final String USER_AGENT_VALUE = LakeClientV1.class.getSimpleName() +
            "/" +
            firstNonNull(LakeClientV1.class.getPackage().getImplementationVersion(), "jvm-unknown");
    ...
    // client session
    private final AtomicReference<LakeSession> lakeSession;
    private String nodeID;
    private final AtomicReference<QueryResults> currentResults = new AtomicReference<>(null);
    private final Consumer<LakeSession> on_session_state_update;

    public LakeClientV1(OkHttpClient httpClient, String sql, ClientSettings settings,
                            Consumer<LakeSession> on_session_state_update,
        ...
        this.lakeSession = new AtomicReference<>(settings.getSession());
    ...
    private Request buildQueryRequest(String query, ClientSettings settings) {
        ...
        LakeSession session = lakeSession.get();
    ...
    private void processResponse(Headers headers, QueryResults results) {
        ...
        LakeSession session = results.getSession();
        if (session != null) {
            lakeSession.set(session);
            if (this.on_session_state_update != null) {
                this.on_session_state_update.accept(session);
            }
        }
        if (results.getQueryId() != null && this.additionalHeaders.get(ClientSettings.X_Lake_Query_ID) == null) {
            this.additionalHeaders.put(ClientSettings.X_Lake_Query_ID, results.getQueryId());
        }
        if (headers != null) {
    ...
    @Override
    public LakeSession getSession() {
        return lakeSession.get();
    }
