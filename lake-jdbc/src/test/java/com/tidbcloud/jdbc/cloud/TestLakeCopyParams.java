...
public class TestLakeCopyParams {
    @Test(groups = {"UNIT"})
    public void testLakeStage() {
        LakeCopyParams prms = LakeCopyParams.builder()
                .setPattern("*.csv")
                .setType("XML")
                .build();
        ...
        prms = LakeCopyParams.builder().build();
        ...
        LakeStage s = LakeStage.builder().stageName("~").path("jdbc/c2/").build();
        prms = LakeCopyParams.builder().setFiles(files).setLakeStage(s).setPattern("a.txt").setType("parquet")
                .setCopyOptions(copyOptions).setFileOptions(fileOptions).build();
        assertEquals(prms.getLakeStage().getStageName(), "~");
        assertEquals(prms.getLakeStage().getPath(), "jdbc/c2/");
