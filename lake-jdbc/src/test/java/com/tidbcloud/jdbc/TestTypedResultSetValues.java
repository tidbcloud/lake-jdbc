...
public class TestTypedResultSetValues {
    ...
    public void testIntervalValueRoundTripsThroughResultSet() throws Exception {
        List<QueryRowField> schema = Collections.singletonList(new QueryRowField("a", new LakeRawType("Interval")));
        LakeUnboundQueryResultSet resultSet = new LakeUnboundQueryResultSet(Optional.empty(), schema,
    ...
    public void testTypedTimestampValueUsesDirectObjectConversion() throws Exception {
        ...
        List<QueryRowField> schema = Arrays.asList(
                new QueryRowField("ts", new LakeRawType("Timestamp")),
                new QueryRowField("d", new LakeRawType("Date"))
        );
        LakeUnboundQueryResultSet resultSet = new LakeUnboundQueryResultSet(Optional.empty(), schema,
