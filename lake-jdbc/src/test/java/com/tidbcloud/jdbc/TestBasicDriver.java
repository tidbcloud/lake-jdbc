...
//    }
    ...
    public void testExecuteUpdateWithNoGeneratedKeys() throws SQLException {
        if (!Compatibility.driverIsGreaterThan("0.4.6")) {
            System.out.println("Skip testExecuteUpdateWithNoGeneratedKeys: driver version too low");
            return;
        }
        try (Connection connection = Utils.createConnection();
             Statement statement = connection.createStatement()) {
    ...
    public void testGeneratedKeysAreNotSupported() throws SQLException {
        if (!Compatibility.driverIsGreaterThan("0.4.6")) {
            System.out.println("Skip testGeneratedKeysAreNotSupported: driver version too low");
            return;
        }
        try (Connection connection = Utils.createConnection();
             Statement statement = connection.createStatement()) {
