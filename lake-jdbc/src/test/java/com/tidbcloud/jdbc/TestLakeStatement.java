...
public class TestLakeStatement {
    @Test(groups = {"UNIT"})
    public void testShouldUseSyntheticResultSetWhenNoSchemaAndNoData() throws SQLException {
        ...
        Assert.assertTrue(LakeStatement.shouldUseSyntheticResultSet("create table t(a int)", queryPages, 0));
    }

    @Test(groups = {"UNIT"})
    public void testShouldNotUseSyntheticResultSetWhenSchemaPresent() throws SQLException {
        List<QueryRowField> schema = Collections.singletonList(new QueryRowField("a", new LakeRawType("Int32")));
        FakeQueryResultPages queryPages = new FakeQueryResultPages(
                Collections.<QueryRowField>emptyList(),
                emptyResults(schema, Collections.<String, String>emptyMap()));

        Assert.assertFalse(LakeStatement.shouldUseSyntheticResultSet("create table t(a int)", queryPages, 0));
    }
    ...
    public void testRealEmptyResultSetRetainsMetadataWhenSchemaPresent() throws SQLException {
        ...
        LakeResultSet resultSet = LakeResultSet.create(newStatementStub(), queryPages, 0, new Capability(new Semver("1.2.900")));
        try {
    ...
    private static Statement newStatementStub() {
        return (Statement) Proxy.newProxyInstance(
                TestLakeStatement.class.getClassLoader(),
