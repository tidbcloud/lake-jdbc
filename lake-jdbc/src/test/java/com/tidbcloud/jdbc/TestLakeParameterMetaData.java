...
public class TestLakeParameterMetaData {
    ...
    public void testGetParameterMetaData() throws SQLException {
        try (Connection conn = Utils.createConnection();
            ...
            for (PreparedStatement ps : new PreparedStatement[]{inputPs, sqlPs}) {
                ...
                Assert.assertEquals(ps.getParameterMetaData().getParameterTypeName(3), LakeDataType.STRING.name().toLowerCase());
            }
        }
        ...
        try (Connection conn = Utils.createConnection();
            ...
            Assert.assertEquals(ps.getParameterMetaData().getParameterTypeName(2), LakeDataType.INT_32.getDisplayName().toLowerCase());
        }
        ...
        try (Connection conn = Utils.createConnection();
            ...
            Assert.assertEquals(ps.getParameterMetaData().getParameterTypeName(2), LakeDataType.VARIANT.getDisplayName().toLowerCase());
        }
    }
}
