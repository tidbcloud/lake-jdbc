...
public class TestPresignContext {
    ...
    public void TestGetPresignUrl() throws SQLException {
        try (Connection connection =  Utils.createConnection()) {
            PresignContext ctx = PresignContext.getPresignContext((LakeConnection) connection, PresignContext.PresignMethod.UPLOAD, null, "test.csv");
    ...
    public void TestGetPresignUrlCase2() throws SQLException {
        try (Connection connection = Utils.createConnection()) {
            ...
            PresignContext ctx = PresignContext.getPresignContext((LakeConnection) connection, PresignContext.PresignMethod.UPLOAD, stageName, "a/b/d/test.csv");
            Assert.assertNotNull(ctx);
            Assert.assertNotNull(ctx.getUrl());
            Assert.assertNotNull(ctx.getHeaders());
        }
