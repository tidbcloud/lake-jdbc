...
import com.tidbcloud.jdbc.internal.session.QueryRequestConfig;
...
public class TestRestQueryResultPages {
    @Test(groups = {"UNIT"})
    public void testMalformedJsonResponseRaisesLakeQueryException() throws Exception {
        ...
        try {
            LakeQueryException exception = Assert.expectThrows(LakeQueryException.class, () ->
            ...
            Assert.assertTrue(exception.getCause().getMessage().contains("Unable to create class com.tidbcloud.jdbc.internal.query.QueryResults"),
    ...
    @Test(groups = {"UNIT"})
    public void testServerQueryErrorResponseRaisesLakeQueryException() throws Exception {
        HttpServer server = HttpServer.create(new InetSocketAddress(0), 0);
        server.createContext("/v1/query", exchange -> {
            try {
                byte[] payload = ("{"
    ...
    public void testMalformedArrowResponseRaisesLakeQueryException() throws Exception {
        HttpServer server = HttpServer.create(new InetSocketAddress(0), 0);
        server.createContext("/v1/query", exchange -> {
            try {
    ...
    @Test(groups = {"UNIT"})
    public void testAdvanceMalformedNextPageRaisesLakeQueryException() throws Exception {
        ...
        try {
            ...
            LakeQueryException exception = Assert.expectThrows(LakeQueryException.class, pages::advance);
    ...
    @Test(groups = {"UNIT"})
    public void testAdvanceRetryable503ExhaustionRaisesLakeQueryException() throws Exception {
        AtomicReference<String> initialQueryId = new AtomicReference<>();
    ...
    @Test(groups = {"UNIT"})
    public void testInitialQueryRetryExhaustionOnNetworkErrorRaisesLakeQueryException() {
        AtomicInteger attempts = new AtomicInteger();
        OkHttpClient client = new OkHttpClient.Builder()
                .addInterceptor((Interceptor) chain -> {
