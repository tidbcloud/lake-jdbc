...
public class LakeDatabaseMetaData implements DatabaseMetaData {
    private static final String SEARCH_STRING_ESCAPE = "\\";
    private final LakeConnection connection;

    public LakeDatabaseMetaData(LakeConnection connection)
            throws SQLException {
    ...
    public String getURL()
            throws SQLException {
        return "jdbc:lake://" + connection.getURI().toString();
    }
    ...
    public ResultSet getTableTypes()
        ...
        return new LakeUnboundQueryResultSet(Optional.ofNullable(connection.createStatement()), schema, results.iterator());
    }
    ...
    private static final List<QueryRowField> META_ROW_FIELDS = new ArrayList<QueryRowField>() {{
        add(new QueryRowField("TABLE_CAT", new LakeRawType("String")));// 1
        add(new QueryRowField("TABLE_SCHEM", new LakeRawType("String")));// 2
//        add(new QueryRowField("TABLE_TYPE", new LakeRawType("String")));// 3
        add(new QueryRowField("TABLE_NAME", new LakeRawType("String")));// 4
        add(new QueryRowField("COLUMN_NAME", new LakeRawType("String")));// 5
        add(new QueryRowField("DATA_TYPE", new LakeRawType("Int32")));// 6
        add(new QueryRowField("TYPE_NAME", new LakeRawType("String")));// 7
        add(new QueryRowField("COLUMN_SIZE", new LakeRawType("Int64")));// 8
        add(new QueryRowField("BUFFER_LENGTH", new LakeRawType("Int64")));// 9
        add(new QueryRowField("DECIMAL_DIGITS", new LakeRawType("Int64")));// 10
        add(new QueryRowField("NUM_PREC_RADIX", new LakeRawType("Int64")));// 11
        add(new QueryRowField("NULLABLE", new LakeRawType("Int64")));// 12
        add(new QueryRowField("REMARKS", new LakeRawType("String")));// 13
        add(new QueryRowField("COLUMN_DEF", new LakeRawType("String")));// 14
        add(new QueryRowField("SQL_DATA_TYPE", new LakeRawType("Int32")));// 15
        add(new QueryRowField("SQL_DATETIME_SUB", new LakeRawType("Int32")));// 16
        add(new QueryRowField("CHAR_OCTET_LENGTH", new LakeRawType("Int32")));// 17
        add(new QueryRowField("ORDINAL_POSITION", new LakeRawType("Int32")));// 18
        add(new QueryRowField("IS_NULLABLE", new LakeRawType("String")));// 19
        add(new QueryRowField("SCOPE_CATALOG", new LakeRawType("String")));// 20
        add(new QueryRowField("SCOPE_SCHEMA", new LakeRawType("String")));// 21
        add(new QueryRowField("SCOPE_TABLE", new LakeRawType("String")));// 22
        add(new QueryRowField("SOURCE_DATA_TYPE", new LakeRawType("Int16")));// 23
        add(new QueryRowField("IS_AUTOINCREMENT", new LakeRawType("String")));// 24
        add(new QueryRowField("IS_GENERATEDCOLUMN", new LakeRawType("String")));// 25
    }};
    ...
    private ResultSet getColumnsMetaDataBySQL(String sql) throws SQLException {
        ...
        try (ResultSet rs = select(sql)) {
            while (rs.next()) {
                ...
                LakeRawType rowType = new LakeRawType(originType);
                LakeDataType dataType = rowType.getDataType();
                // DATA_TYPE
                ...
                if (dataType == LakeDataType.STRING) {
        ...
        return new LakeUnboundQueryResultSet(Optional.ofNullable(connection.createStatement()),
    ...
    // input LakeQuery v0.8.173-nightly-d66d905(rust-1.67.0-nightly-2023-01-03T08:02:54.266305248Z)
    // return 8 use regex
    @Override
    ...
    private ResultSet select(String sql)
        ...
        LakeResultSet resultSet;
        try {
            resultSet = (LakeResultSet) statement.executeQuery(sql);
