...
import com.tidbcloud.jdbc.internal.http.JsonResponse;
...
public class RestQueryResultPages implements QueryResultPages {
    ...
    public RestQueryResultPages(OkHttpClient httpClient, String sql, QueryRequestConfig requestConfig, Consumer<SessionState> onSessionStateUpdate, AtomicReference<String> lastNodeID) {
        ...
        if (!completed) {
            throw new LakeQueryException("Query failed to complete");
        }
    ...
    private boolean executeInternal(Request request) {
        ...
        try {
            ...
            if (payload.statusCode == HTTP_OK && payload.results != null) {
                ...
                throw new LakeQueryException("Query Failed: " + error);
            }
            return false;
        } catch (IllegalArgumentException e) {
            throw new LakeQueryException("Failed to decode query response", e);
        } catch (SQLException e) {
            throw new LakeQueryException("Failed to execute query request", e);
        }
    }
    ...
    private ResponsePayload decodeArrowResponse(HttpRetryPolicy.ResponseWithBody responseWithBody) throws SQLException {
        BufferAllocator allocator = rootAllocator().newChildAllocator("lake-jdbc-arrow-page", 0, Long.MAX_VALUE);
        try (ArrowStreamReader reader = new ArrowStreamReader(
            ...
            if (responseHeader == null) {
                throw new LakeQueryException("Missing response_header metadata in Arrow payload");
            }
