...
import com.tidbcloud.jdbc.exception.LakeUnsupportedOperationException;
...
import static com.tidbcloud.jdbc.LakeColumnInfo.setTypeInfo;
import static java.lang.Math.toIntExact;
import static java.lang.String.format;
import static java.util.Locale.ENGLISH;
import static java.util.Objects.requireNonNull;

abstract class AbstractLakeResultSet implements ResultSet {
    ...
    private static final DateTimeFormatter TIMESTAMP_TZ_PATTERN = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSSSSS XX");
    private static final DateTimeFormatter TIMESTAMP_PATTERN = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSSSSS");
    ...
    private final List<LakeColumnInfo> lakeColumnInfoList;
    ...
    AbstractLakeResultSet(Optional<Statement> statement, List<QueryRowField> schema, Iterator<List<Object>> results, Map<String, String> resultSetting, String queryId) {
        this.statement = requireNonNull(statement, "statement is null");
        this.fieldMap = getFieldMap(schema);
        this.lakeColumnInfoList = getColumnInfo(schema);
        this.results = requireNonNull(results, "results is null");
        this.resultSetMetaData = new LakeResultSetMetaData(lakeColumnInfoList);
    ...
    private static List<LakeColumnInfo> getColumnInfo(List<QueryRowField> columns) {
        ImmutableList.Builder<LakeColumnInfo> list = ImmutableList.builderWithExpectedSize(columns.size());
        for (QueryRowField column : columns) {
            LakeColumnInfo.Builder builder = new LakeColumnInfo.Builder()
                    // TODO
                    .setCatalogName("")
                    // TODO
                    .setSchemaName("")
    ...
    public boolean next() throws SQLException {
        ...
        try {
            ...
            if (sqlException != null) {
                throw new LakeSQLException(sqlException, queryId);
            }
            throw new LakeSQLException("error fetching results", queryId, e);
        }
    }
    ...
    private Timestamp getTimestamp(int columnIndex, ZoneId localTimeZone)  throws SQLException {
        ...
        if (temporal.isSupported(ChronoField.OFFSET_SECONDS)) {
            ...
            return Timestamp.from(localDt.atOffset(offset).toInstant());
        } else {
            return Timestamp.from(localDt.atZone(localTimeZone).toInstant());
        }
    }
    ...
    public int columnIndex(String label)
            throws SQLException {
        if (label == null) {
            throw new LakeSQLException("Column label is null", queryId);
        }
        Integer index = fieldMap.get(label.toLowerCase(ENGLISH));
        if (index == null) {
            throw new LakeSQLException("Invalid column label: " + label + ". Valid column labels are: " + fieldMap.keySet(), queryId);
        }
        return index;
    }
    ...
    public Object getObject(int columnIndex)
        ...
        LakeRawType lakeRawType = this.lakeColumnInfoList.get(columnIndex - 1).getType();
        if (LakeRawType.startsWithIgnoreCase(lakeRawType.getType(), LakeTypes.INTERVAL)) {
            if (!(value instanceof String)) {
    ...
    public int getHoldability() throws SQLException {
        throw new LakeUnsupportedOperationException();
    }
    ...
    OffsetDateTime getOffsetDateTimeNotNull(String value, LakeRawType type) throws SQLException{
        LakeDataType dataType = type.getDataType();
        if (dataType == LakeDataType.TIMESTAMP_TZ) {
            return OffsetDateTime.parse(value, TIMESTAMP_TZ_PATTERN);
        }
        if (dataType == LakeDataType.TIMESTAMP) {
   ...
   ZonedDateTime getZonedDateTimeNotNull(String value, LakeRawType type) throws SQLException{
        if (type.getDataType() == LakeDataType.TIMESTAMP) {
    ...
    static String invalidColumnType(LakeRawType data, String exp) {
        return String.format("Can not get %s as %s", data.getType(), exp);
    }
    ...
    public <T> T getObject(int columnIndex, Class<T> type)
            throws SQLException {
        if (type == null) {
            throw new SQLDataException("type is null");
        }
        Object value = column(columnIndex);
        if (value == null) {
            return null;
        ...
        if (type == Instant.class) {
            return type.cast( getTimestamp(columnIndex, resultTimeZone).toInstant());
        }

        if (type == OffsetDateTime.class) {
            return type.cast(getOffsetDateTimeNotNull((String) value, lakeRawType));
        }

        if (type == ZonedDateTime.class) {
            return type.cast(getZonedDateTimeNotNull((String) value, lakeRawType));
        }
        ...
        if (type == Duration.class) {
            ...
            if (!(intervalValue instanceof Duration)) {
                throw new SQLDataException(invalidColumnType(lakeRawType, "Duration"));
            }
