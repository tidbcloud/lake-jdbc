## (Experimental) Lake Trino Connector User Guide
...
#### Install Experimental Lake Trino Connector

Currently, the Lake Trino connector is awaiting Trino community review: https://github.com/trinodb/trino/pull/23024

To try it out, you could download the target [zip](https://repo.lake.com/trino/trino-lake-454.zip) and unzip it to the `lake` directory in
`/data/trino/plugin`:
...
To build the Lake connector from scratch, clone the Lake repository and build the connector:
...
```properties
connector.name=lake
connection-url=jdbc:lake://host.default.lake.com:443?ssl=true
...
#### Trino to Lake Type Mapping

| Trino Type                  | Lake Type | Notes |
|-----------------------------|---------------|-------|
| BOOLEAN                     | TINYINT       |       |
| TINYINT                     | TINYINT       |       |
| SMALLINT                    | SMALLINT      |       |
...
The connector provides specific table functions to access Lake.
...
The connector provides read access and write access to data and metadata in the Lake database.
...
The Lake connector provides a schema for every Lake database. You can see the available Lake databases by
